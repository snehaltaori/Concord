...
const Forum = () => {
    ...
    const fetchThreadsAndReplies = async () => {
      try {
        ...
        const threadsWithReplies = threads.map((thread) => ({
          ...thread,
          replies:
            replies.filter((reply) => reply.threadId === thread.threadId) || [],
          showReplyForm: false,
          showReplies: false,
        }));
    ...
    if (newThreadTitle && newThreadDescription && selectedCategory) {
      ...
      axios
        ...
        .catch((error) => {
          ...
          alert(
            "An error occurred while posting the thread. Please try again later."
          );
        });
    } else {
  ...
  const searchThreads = () => {
    if (searchTerm.trim() === "") {
      ...
      const filteredThreads = originalThreads.filter(
        (thread) =>
      ...
      >
        <section className="main ">
          ...
          <p className="text-center text-white mb-6">
            Discuss and collaborate on organizational matters related to
            college.
          </p>
          <a className="a-vishal" href="../Dashboard/index.html">
            Back to Dashboard
          </a>
                  ...
                  <ul className="">
                    {categories.map((category, index) => (
                      <li
                        ...
                        onClick={() =>
                          filterThreadsByCategory(categories[index])
                        }
                      >
                        <div
                          className={`py-2 px-2 ${
